...
mai <- mai_plot %>%
  ...
  st_drop_geometry() %>%
  select(NAME_1, NAME_2, NAME_3, pro.pop, prop.mai, lng, lat, distm_km, rank)

mai_peak <- mai_plot %>% filter(NAME_3=="Thi tran Nga Son")

datatable(mai, colnames = c("Tỉnh/Thành phố", "Quận/Huyện", "Phường/Xã", "Tỉ lệ/xã phường", "Tỉ lệ/cả nước", "Kinh độ", "Vĩ độ", "Khoảng cách tới tâm (km)", "STT"), extensions = 'Buttons', options = list(
    dom = 'Bfrtip',
...
phung <- phung_plot %>%
  ...
  mutate(rank=row_number()) %>%
  st_drop_geometry() %>%
  select(NAME_1, NAME_2, NAME_3, pro.pop, prop.phung, lng, lat, distm_km, rank)

phung_peak <- phung_plot %>% filter(NAME_3=="Xa Phu Son" & NAME_2=="Ba Vi")

datatable(phung, colnames = c("Tỉnh/Thành phố", "Quận/Huyện", "Phường/Xã", "Tỉ lệ/xã phường", "Tỉ lệ/cả nước", "Kinh độ", "Vĩ độ", "Khoảng cách tới tâm (km)", "STT"), extensions = 'Buttons', options = list(
    dom = 'Bfrtip',
    buttons = c('copy', 'csv', 'excel', 'pdf', 'print')
  ))
```
